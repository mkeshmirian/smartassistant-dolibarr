...
Thin, open-source (GPLv3+) connector that embeds the Smart Assistant dashboard
in Dolibarr and connects your instance to the hosted AI service.
...
GPLv3 or later (full text in `LICENSE` at the repo root, also shipped as
`docs/COPYING`). The hosted service (Smart Assistant engine) is a separate,
proprietary product; access is controlled by tokens issued by the service
provider.
