...
| **State storage** | SQLite inside the service (instance registrations, agent state, recommendations) |
...
- Current limitation: instance data lives in the container's SQLite (ephemeral across
  service redeploys; reconnecting restores service). A durable Firestore-backed store is
  on the roadmap.
